...
class PythonGroqProvider:
    ...
    def extract_structured_record(self, payload: dict) -> dict:
        ...
        system_prompt = (
            ...
            "3. LANGUAGE & TRANSLATION: All extracted field values (districtName, mandalName, villageName, ownerName, fatherOrHusbandName, landClassification, boundaries, etc.) MUST be returned in standard ENGLISH. Translate or transliterate Telugu values to English (e.g. 'కర్నూలు' -> 'Kurnool', 'ఎస్. రమేష్' -> 'S. Ramesh', 'పట్టా భూమి' -> 'Patta Land / Dry Land', 'ఉల్చాల' -> 'Ulchala', 'కర్నూలు రూరల్' -> 'Kurnool Rural'). Keep original Telugu snippets in 'evidence' fields.\n"
            "4. Output MUST be valid JSON with fields: documentType, documentTitle, documentTitleTe, overallConfidence, districtName, districtConfidence, districtEvidence, mandalName, mandalConfidence, mandalEvidence, villageName, villageConfidence, villageEvidence, surveyNumber, surveyConfidence, surveyEvidence, subDivisionNumber, subDivisionConfidence, khataNumber, khataConfidence, ownerName, ownerConfidence, ownerEvidence, fatherOrHusbandName, fatherConfidence, extentAcres, extentConfidence, landClassification, classificationConfidence, documentDate, boundaries, customSections, checklist."
        )
